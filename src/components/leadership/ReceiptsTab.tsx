    ...
    <div className="space-y-6">
      ...
      {selectedReceipt && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4">
          ...
          <div className="relative bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-3xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6">
              ...
              {(selectedReceipt.notes || selectedReceipt.employee_comment) && (
                <div className="mb-4 sm:mb-6 p-3 bg-slate-50 rounded-lg">
              ...
              <div className="mb-4 sm:mb-6">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-lg bg-gradient-to-br from-teal-500 to-cyan-600 flex items-center justify-center shadow-sm">
                      <Sparkles className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white" />
                    </div>
                    <div>
                      <p className="text-xs sm:text-sm font-semibold text-slate-800">AI Receipt Analysis</p>
                      <p className="text-[9px] sm:text-[10px] text-slate-500">Powered by GPT-4 Vision</p>
                    </div>
                  </div>
                  {analysis && !analyzingReceipt && (
                    <button
                      onClick={reanalyzeReceipt}
                      className="inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 text-[10px] sm:text-xs font-medium text-teal-700 bg-teal-50 hover:bg-teal-100 rounded-lg transition-colors"
                    >
                      ...
                      <div className="flex items-start sm:items-center justify-between gap-2 sm:gap-4">
                        <div className="flex items-center gap-2 sm:gap-3">
                          <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center shadow-sm flex-shrink-0 ${
                            ...
                            {analysis.confidence_score >= 90 ? (
                              ...
                              <ShieldQuestion className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
                            )}
                          </div>
                        ...
                        <div className="text-right flex-shrink-0">
                          <div className="flex items-center gap-1.5 sm:gap-2 mb-1">
                            <span className="text-[10px] sm:text-xs font-medium text-slate-600 hidden sm:inline">Score</span>
                          ...
                          <div className="w-16 sm:w-28 h-2 sm:h-2.5 bg-slate-200 rounded-full overflow-hidden">
                            <div
                    ...
                    <div className="p-3 sm:p-5 bg-white space-y-3 sm:space-y-4">
                      ...
                      <div className="space-y-2 sm:space-y-3">
                        <div className={`p-3 sm:p-4 rounded-xl border-l-4 ${
                          ...
                          <div className="flex items-start justify-between mb-2">
                            <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                              <span className="text-[10px] sm:text-xs font-bold text-slate-700 uppercase tracking-wide">Vendor</span>
                              {analysis.vendor_match ? (
                                <span className="px-1.5 sm:px-2 py-0.5 text-[9px] sm:text-[10px] font-semibold bg-emerald-100 text-emerald-700 rounded-full">MATCH</span>
                              ) : (
                                <span className="px-1.5 sm:px-2 py-0.5 text-[9px] sm:text-[10px] font-semibold bg-red-100 text-red-700 rounded-full">MISMATCH</span>
                              )}
                            </div>
                          </div>
                          <div className="grid grid-cols-2 gap-2 sm:gap-4 mb-2 sm:mb-3">
                            <div className="bg-white/60 rounded-lg p-1.5 sm:p-2">
                              <p className="text-[9px] sm:text-[10px] text-slate-500 uppercase mb-0.5">Expected</p>
                              <p className="text-xs sm:text-sm font-semibold text-slate-800 truncate">{analysis.expected_vendor || selectedReceipt.purchase_request?.vendor_name}</p>
                            </div>
                            <div className="bg-white/60 rounded-lg p-1.5 sm:p-2">
                              <p className="text-[9px] sm:text-[10px] text-slate-500 uppercase mb-0.5">Found</p>
                              <p className="text-xs sm:text-sm font-semibold text-slate-800 truncate">{analysis.extracted_vendor || 'Not detected'}</p>
                            </div>
                          </div>
                          <div className="flex items-start gap-1.5 sm:gap-2 bg-white/80 rounded-lg p-1.5 sm:p-2">
                            <Sparkles className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-teal-600 mt-0.5 flex-shrink-0" />
                        ...
                        <div className={`p-3 sm:p-4 rounded-xl border-l-4 ${
                          ...
                          <div className="flex items-start justify-between mb-2">
                            <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                              <span className="text-[10px] sm:text-xs font-bold text-slate-700 uppercase tracking-wide">Amount</span>
                              {analysis.amount_match ? (
                                <span className="px-1.5 sm:px-2 py-0.5 text-[9px] sm:text-[10px] font-semibold bg-emerald-100 text-emerald-700 rounded-full">MATCH</span>
                              ) : (
                                <span className="px-1.5 sm:px-2 py-0.5 text-[9px] sm:text-[10px] font-semibold bg-red-100 text-red-700 rounded-full">MISMATCH</span>
                          ...
                          <div className="grid grid-cols-2 gap-2 sm:gap-4 mb-2 sm:mb-3">
                            <div className="bg-white/60 rounded-lg p-1.5 sm:p-2">
                              <p className="text-[9px] sm:text-[10px] text-slate-500 uppercase mb-0.5">Expected</p>
                              <p className="text-xs sm:text-sm font-semibold text-slate-800">{formatCurrency(analysis.expected_amount || selectedReceipt.purchase_request?.total_amount || 0)}</p>
                            </div>
                            <div className="bg-white/60 rounded-lg p-1.5 sm:p-2">
                              <p className="text-[9px] sm:text-[10px] text-slate-500 uppercase mb-0.5">Found</p>
                              <p className="text-xs sm:text-sm font-semibold text-slate-800">{analysis.extracted_amount !== null ? formatCurrency(analysis.extracted_amount) : 'Not detected'}</p>
                            </div>
                          </div>
                          <div className="flex items-start gap-1.5 sm:gap-2 bg-white/80 rounded-lg p-1.5 sm:p-2">
                            <Sparkles className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-teal-600 mt-0.5 flex-shrink-0" />
                        ...
                        <div className={`p-3 sm:p-4 rounded-xl border-l-4 ${
                          ...
                          <div className="flex items-start justify-between mb-2">
                            <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                              <span className="text-[10px] sm:text-xs font-bold text-slate-700 uppercase tracking-wide">Date</span>
                              {analysis.date_match ? (
                                <span className="px-1.5 sm:px-2 py-0.5 text-[9px] sm:text-[10px] font-semibold bg-emerald-100 text-emerald-700 rounded-full">MATCH</span>
                              ) : (
                                <span className="px-1.5 sm:px-2 py-0.5 text-[9px] sm:text-[10px] font-semibold bg-amber-100 text-amber-700 rounded-full">REVIEW</span>
                              )}
                            </div>
                          </div>
                          <div className="grid grid-cols-2 gap-2 sm:gap-4 mb-2 sm:mb-3">
                            <div className="bg-white/60 rounded-lg p-1.5 sm:p-2">
                              <p className="text-[9px] sm:text-[10px] text-slate-500 uppercase mb-0.5">Expected</p>
                              <p className="text-xs sm:text-sm font-semibold text-slate-800">{analysis.expected_date ? formatDate(analysis.expected_date) : formatDate(selectedReceipt.purchase_request?.expense_date || '')}</p>
                            </div>
                            <div className="bg-white/60 rounded-lg p-1.5 sm:p-2">
                              <p className="text-[9px] sm:text-[10px] text-slate-500 uppercase mb-0.5">Found</p>
                              <p className="text-xs sm:text-sm font-semibold text-slate-800">{analysis.extracted_date ? formatDate(analysis.extracted_date) : 'Not detected'}</p>
                            </div>
                          </div>
                          <div className="flex items-start gap-1.5 sm:gap-2 bg-white/80 rounded-lg p-1.5 sm:p-2">
                            <Sparkles className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-teal-600 mt-0.5 flex-shrink-0" />
                      ...
                      {analysis.extracted_items && analysis.extracted_items.length > 0 && (
                        <div className="bg-slate-50 rounded-xl p-3 sm:p-4">
                          <p className="text-[10px] sm:text-xs font-bold text-slate-700 uppercase tracking-wide mb-2 sm:mb-3">Items Detected</p>
                          <div className="space-y-1.5 sm:space-y-2">
                            {analysis.extracted_items.slice(0, 3).map((item, idx) => (
                              <div key={idx} className="flex items-center justify-between text-xs sm:text-sm bg-white px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg">
                                <span className="text-slate-700 truncate flex-1">{item.description}</span>
                                <span className="font-medium text-slate-900 ml-2 sm:ml-3">{formatCurrency(item.amount)}</span>
                              </div>
                            ))}
                            {analysis.extracted_items.length > 3 && (
                              <p className="text-[10px] sm:text-xs text-slate-500 text-center pt-1">
                                +{analysis.extracted_items.length - 3} more items
                              </p>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="bg-gradient-to-r from-slate-50 to-slate-100 rounded-xl p-3 sm:p-4">
                        <div className="flex items-start gap-2 sm:gap-3">
                          ...
                          <div className="min-w-0">
                            <p className="text-[10px] sm:text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">AI Summary</p>
                            <p className="text-xs sm:text-sm text-slate-700 leading-relaxed">{analysis.analysis_notes}</p>
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            {view === 'review' && selectedReceipt.status !== 'approved' ? (
              <div className="px-4 sm:px-6 py-4 border-t border-slate-100 flex-shrink-0 bg-white">
                <button
                  onClick={() => handleResponse('approved')}
                  disabled={submitting}
                  className="w-full flex items-center justify-center gap-2 px-4 py-3 text-base font-semibold text-white bg-emerald-600 rounded-xl hover:bg-emerald-700 transition-colors disabled:opacity-50 shadow-lg shadow-emerald-200 mb-3"
                >
                  <CheckCircle className="w-5 h-5" />
                  {submitting ? 'Processing...' : 'Approve Receipt'}
                </button>
                <div className="grid grid-cols-3 gap-2">
                  <button
                    onClick={() => setShowReuploadModal(true)}
                    disabled={submitting || selectedReceipt.reupload_requested}
                    className="flex flex-col items-center justify-center gap-1 px-2 py-2.5 text-xs font-medium text-orange-700 bg-orange-50 rounded-lg hover:bg-orange-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <Upload className="w-4 h-4" />
                    <span className="text-center leading-tight">{selectedReceipt.reupload_requested ? 'Requested' : 'Re-upload'}</span>
                  </button>
                  <button
                    onClick={() => handleResponse('needs_info')}
                    disabled={submitting || !responseComment.trim()}
                    className="flex flex-col items-center justify-center gap-1 px-2 py-2.5 text-xs font-medium text-sky-700 bg-sky-50 rounded-lg hover:bg-sky-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <HelpCircle className="w-4 h-4" />
                    <span className="text-center leading-tight">Request Info</span>
                  </button>
                  <button
                    onClick={() => handleResponse('rejected')}
                    disabled={submitting || !responseComment.trim()}
                    className="flex flex-col items-center justify-center gap-1 px-2 py-2.5 text-xs font-medium text-red-700 bg-red-50 rounded-lg hover:bg-red-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <XCircle className="w-4 h-4" />
                    <span className="text-center leading-tight">Reject</span>
                  </button>
                </div>
              </div>
            ) : (
              <div className="px-4 sm:px-6 py-4 border-t border-slate-100 flex items-center justify-end flex-shrink-0">
                <button
                  onClick={() => setSelectedReceipt(null)}
