/*
  ...
  Drop and recreate admin functions to ensure no duplicates exist
*/

DROP FUNCTION IF EXISTS get_pcard_monthly_usage();
DROP FUNCTION IF EXISTS admin_reset_pcard_limit(numeric);
DROP FUNCTION IF EXISTS admin_clear_all_purchases(text);
DROP FUNCTION IF EXISTS admin_reset_monthly_usage();
DROP FUNCTION IF EXISTS admin_set_hard_stop(boolean);

CREATE FUNCTION get_pcard_monthly_usage()
RETURNS jsonb AS $$
DECLARE
...
CREATE FUNCTION admin_reset_pcard_limit(new_limit numeric)
RETURNS jsonb AS $$
DECLARE
  is_admin boolean;
  old_limit numeric;
...
CREATE FUNCTION admin_clear_all_purchases(confirm_text text)
RETURNS jsonb AS $$
DECLARE
  is_admin boolean;
...
CREATE FUNCTION admin_reset_monthly_usage()
RETURNS jsonb AS $$
DECLARE
  is_admin boolean;
  current_month_start date;
...
CREATE FUNCTION admin_set_hard_stop(enabled boolean)
RETURNS jsonb AS $$
DECLARE
  is_admin boolean;
  settings_id uuid;
